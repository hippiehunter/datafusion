...
use datafusion_expr::{AutoGenerate, Expr, col};
...
impl<'a, S: ContextProvider> SqlToRel<'a, S> {
    ...
    /// Extracts IDENTITY and SERIAL auto-generation specifications from column definitions.
    ///
    /// Processes column options to find:
    /// - `GENERATED ALWAYS AS IDENTITY` / `GENERATED BY DEFAULT AS IDENTITY`
    /// - `DEFAULT nextval('sequence_name')` (SERIAL pattern)
    ///
    /// Returns a map from column name to its auto-generation specification.
    pub(super) fn build_column_auto_generates(
        &self,
        columns: &[SQLColumnDef],
    ) -> Result<Vec<(String, AutoGenerate)>> {
        use sqlparser::ast::ColumnOption;

        let mut auto_generates = Vec::new();

        for column in columns {
            let col_name = self.ident_normalizer.normalize(column.name.clone());

            for option in &column.options {
                match &option.option {
                    ColumnOption::Identity(identity) => {
                        // Parse GENERATED {ALWAYS|BY DEFAULT} AS IDENTITY [(sequence options)]
                        // Extract start/increment from any identity property with parameters
                        let extract_params = |params: &sqlparser::ast::IdentityParameters| -> (i64, i64) {
                            let start = params.seed.to_string().parse::<i64>().unwrap_or(1);
                            let increment = params.increment.to_string().parse::<i64>().unwrap_or(1);
                            (start, increment)
                        };

                        let (start, increment) = match identity {
                            sqlparser::ast::IdentityPropertyKind::Autoincrement(prop)
                            | sqlparser::ast::IdentityPropertyKind::Identity(prop) => {
                                match &prop.parameters {
                                    Some(sqlparser::ast::IdentityPropertyFormatKind::FunctionCall(params))
                                    | Some(sqlparser::ast::IdentityPropertyFormatKind::StartAndIncrement(params)) => {
