...
pub(crate) fn resolve_positions_to_exprs(
    ...
    match expr {
        expr if literal_position(&expr).is_some() => {
            let Some(position) = literal_position(&expr) else {
                return Ok(expr);
            };
            if position > 0_i64 && position <= select_exprs.len() as i64 {
                let index = (position - 1) as usize;
                let select_expr = &select_exprs[index];
                return Ok(match select_expr {
...
mod tests {
    ...
    // Note: test_transform_non_consecutive_unnests is disabled because it uses
    // FieldAccessor from datafusion_functions which was removed from this workspace.
    use indexmap::IndexMap;
    ...
    // test_transform_non_consecutive_unnests is disabled - uses FieldAccessor from removed crate
