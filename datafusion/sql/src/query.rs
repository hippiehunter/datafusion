...
use datafusion_expr::{
    CreateTableExpr, DdlStatement, Distinct, Expr, LogicalPlan, LogicalPlanBuilder,
};
use sqlparser::ast::{
...
impl<S: ContextProvider> SqlToRel<'_, S> {
    ...
    pub(crate) fn query_to_plan(
        ...
        match set_expr {
            SetExpr::Select(mut select) => {
                ...
                let plan = self.limit(plan, offset, limit, planner_context)?;
    ...
    fn select_into(
        ...
        match select_into {
            Some(into) => Ok(LogicalPlan::Ddl(DdlStatement::CreateTableExpr(
                CreateTableExpr {
                    ...
                    temporary: false,
                    column_defaults: vec![],
                    ast: None, // SELECT INTO doesn't have original CREATE TABLE AST
                },
            ))),
            _ => Ok(plan),
        }
