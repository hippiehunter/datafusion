...
// Note: string::concat from datafusion_functions was removed
...
fn test_incompatible_types_binary_arithmetic() -> Result<()> {
    let query = "SELECT /*whole+left*/id/*left*/ + /*right*/birth_date/*right+whole*/ FROM person";
    let spans = get_spans(query);
    let diag = do_query(query);
...
// Removed: test_invalid_function
// (concat function from pruned datafusion_functions crate unavailable for suggestion)
