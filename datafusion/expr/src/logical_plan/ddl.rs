...
/// Specifies how a column's value is automatically generated.
#[derive(Debug, Clone, PartialEq, Eq, PartialOrd, Hash)]
pub enum AutoGenerate {
    /// GENERATED ALWAYS AS IDENTITY or GENERATED BY DEFAULT AS IDENTITY
    Identity {
        /// If true, GENERATED ALWAYS (user cannot override).
        /// If false, GENERATED BY DEFAULT (user can override).
        always: bool,
        /// Start value for the identity sequence.
        start: i64,
        /// Increment value for the identity sequence.
        increment: i64,
    },
    /// DEFAULT nextval('sequence_name') or SERIAL type
    Sequence {
        /// The name of the sequence used for auto-generation.
...
pub struct CreateTable {
    ...
    /// Auto-generation specifications for columns (IDENTITY, SERIAL, etc.).
    /// Each entry maps a column name to its auto-generation specification.
    pub column_auto_generates: Vec<(String, AutoGenerate)>,
}
